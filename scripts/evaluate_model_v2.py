"""
Model Evaluation Script - V2

Evaluates the v2 house price prediction model performance and generalization.
...
MODEL_FEATURES_PATH = Path("model/model_features_v2.json")
...
RANDOM_STATE = 42  # Must match create_model_v2.py
...
def load_model():
    ...
    try:
        ...
        logger.error("Please run create_model_v2.py first to train the model")
        raise
    except Exception as e:
...
def main():
    ...
    try:
        logger.info("=" * 60)
        logger.info("MODEL EVALUATION REPORT - V2")
        logger.info("=" * 60)
