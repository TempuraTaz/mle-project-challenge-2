"""
Model Evaluation Script - V5

Evaluates the v5 house price prediction model performance and generalization.
V5 uses XGBoost (gradient boosting) with all features - direct comparison to V3 (Random Forest).
"""
...
MODEL_PATH = Path("model/model_v5.pkl")
MODEL_FEATURES_PATH = Path("model/model_features_v5.json")
...
RANDOM_STATE = 42  # Must match create_model_v5.py
...
def load_model():
    ...
    try:
        ...
        logger.error("Please run create_model_v5.py first to train the model")
        raise
    except Exception as e:
...
def main():
    ...
    try:
        logger.info("=" * 60)
        logger.info("MODEL EVALUATION REPORT - V5 (XGBoost)")
        logger.info("=" * 60)
