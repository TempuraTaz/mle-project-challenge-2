"""
Model Evaluation Script - V4

Evaluates the v4 house price prediction model performance and generalization.
V4 uses only top 12 features from V3 feature importance analysis.
"""
...
MODEL_PATH = Path("model/model_v4.pkl")
MODEL_FEATURES_PATH = Path("model/model_features_v4.json")
...
RANDOM_STATE = 42  # Must match create_model_v4.py
...
def load_data():
    ...
    try:
        ...
        # Extract house features needed from CSV
        house_features = ['sqft_living', 'grade', 'sqft_above', 'sqft_living15',
        ...
        # Select only the model features (top 12)
...
def load_model():
    ...
    try:
        ...
        logger.error("Please run create_model_v4.py first to train the model")
        raise
    except Exception as e:
...
def main():
    ...
    try:
        logger.info("=" * 60)
        logger.info("MODEL EVALUATION REPORT - V4 (Simplified)")
        logger.info("=" * 60)
